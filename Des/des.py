...
class DES:
    ...
    def _feistel_function(self, right, subkey):
        ...
        for i in range(8):
            ...
            sbox_result.append((val >> 3) & 1)
            sbox_result.append((val >> 2) & 1)
            sbox_result.append((val >> 1) & 1)
            sbox_result.append(val & 1)
...
# --- Ví dụ sử dụng ---
# Khóa chính 64 bit (định dạng bytes)
key = b'12345678'
# Bản rõ 64 bit (định dạng bytes)
plaintext = b'12345678'
# Tạo đối tượng DES với khóa
des_cipher = DES(key)
# Mã hóa
...
# Kiểm tra xem bản giải mã có khớp với bản rõ ban đầu không
